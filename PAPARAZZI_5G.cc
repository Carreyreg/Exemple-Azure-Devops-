...
CourseChange (Ptr<const MobilityModel> mobility)
{
  ...
  std::cout << simTime << " POS: x=" << pos.x << ", y=" << pos.y << std::endl;
   ...
   std::ofstream out3("energy_paparazzi.csv", std::ios::app);      
...
delayCalcul( Ptr<Ipv4FlowClassifier> classifier, Ptr<FlowMonitor> monitor){
    ...
    std::ofstream out3("paprazzi_5g.csv", std::ios::app);      
...
main(int argc, char* argv[])
{
    ...
    double bandwidthBand1 = 200e6; // largeur de la bande passante 
  ...
  Ptr<ListPositionAllocator> apPositionAlloc = CreateObject<ListPositionAllocator>();
    ...
    mobility.SetMobilityModel ("ns3::PaparazziMobilityModel",
			...
			"Bounds", BoxValue (Box (-10, 100, -10, 100, 0, 10)));
...
//Creation de l'application de streaming 

//port d'ecoute du serveur qui sera installé dans la station de controle
...
// decommentez ça pour avoir la simulation 2D de netanim
