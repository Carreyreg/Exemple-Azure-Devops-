...
delayCalcul( Ptr<Ipv4FlowClassifier> classifier, Ptr<FlowMonitor> monitor){
    ...
    std::ofstream out3("psos_5g.csv", std::ios::app);      
...
main(int argc, char* argv[])
{
 ...
 // Lancement du code python qui genere la mobility 
    const char* command = "python MC-PSO.py";
    std::system(command);
    ...
    double bandwidthBand1 = 250e6; // largeur de la bande passante 
    ...
    Ns2MobilityHelper ns2 = Ns2MobilityHelper ("pso_trace.txt");
...
// decommentez et modifiez ça pour avoir la simulation 2D de netanim
