...
delayCalcul( Ptr<Ipv4FlowClassifier> classifier, Ptr<FlowMonitor> monitor){
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
        ...
        if ((i->second.timeLastRxPacket.GetSeconds()-i->second.timeFirstTxPacket.GetSeconds()) != 0) {
            ...
            Throughput = Throughput + i->second.rxBytes * 8.0/(i->second.timeLastRxPacket.GetSeconds()-i->second.timeFirstTxPacket.GetSeconds())/1024/1024; // en Mbps
    ...
    std::ofstream out3("maga_5g.csv", std::ios::app);      
...
main(int argc, char* argv[])
{
 ...
 //Lancement du code python qui genere la mobility 
    const char* command = "python3 ./mobility/MA_GA.py";
    std::system(command);
    ...
    Ns2MobilityHelper ns2 = Ns2MobilityHelper ("maga_mobility.txt");


    NodeList::Iterator i = NodeList::Begin();
    NodeList::Iterator j = NodeList::End();
    ns2.Install (i,j);
...
//création du client udp qui est installé dans les uavs et qui envoie du traffic vidéo à la station de controle 
    ...
    // Le bearer qui va porter le trafic vidéo 
